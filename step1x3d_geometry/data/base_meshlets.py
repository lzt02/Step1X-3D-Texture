...
class BaseDataset(Dataset):
    ...
    def _load_shape_from_occupancy_or_sdf(self, index: int) -> Dict[str, Any]:
        ...
        elif self.cfg.geo_data_type == "texture":
            data = np.load(f"{self.cfg.root_dir}/meshlets/{self.uids[index]}.npy")
            feature = data
            surface = data #data[:, -3:]
            if self.cfg.with_sharp_data:
                sharp_surface = data["sharp_surface"]
    ...
    def _get_data(self, index):
        ...
        if self.cfg.load_geometry:
            if self.cfg.geo_data_type == "occupancy" or self.cfg.geo_data_type == "sdf" or self.cfg.geo_data_type == "texture":
                # load shape
